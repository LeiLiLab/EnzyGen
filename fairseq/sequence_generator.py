...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        if "src_tokens" in net_input:
            ...
            src_lengths = (
                (src_tokens.ne(self.bos) & src_tokens.ne(self.eos) & src_tokens.ne(self.pad)).long().sum(dim=1)
            )
        ...
        encoder_out = encoder_outs[0]["encoder_out"][0]  # [batch, src_length, dim]
        # esm encoder
        cls_encoder_out = encoder_out[:, 0, :]  # [batch, dim]
        # transformer encoder
        # cls_encoder_out = encoder_out[0, :, :]  # [batch, dim]
        z_mean = self.model.single_model.mean_layer1(cls_encoder_out)  # [batch, dim]
        z_std = torch.exp(0.5 * self.model.single_model.logvar_layer1(cls_encoder_out))  # [batch, dim]
        ...
        latent = noise
        latent = latent.repeat(1, self.beam_size).reshape(-1, latent.size()[1])  # [batch * beam, latent]
    ...
    def forward_decoder(
        ...
        for i, model in enumerate(self.models):
            ...
            if self.has_incremental_states():
                decoder_out = model.decoder.inference_forward(
                    tokens,
                ...
                if hasattr(model, "decoder"):
                    decoder_out = model.decoder.forward(tokens, encoder_out=encoder_out, latent_vectos=latent)
                else:
