...
def save_checkpoint(cfg: CheckpointConfig, trainer, epoch_itr, val_loss, save_metadata=False):
    ...
    if not end_of_epoch and cfg.keep_interval_updates > 0:
        ...
        if cfg.keep_interval_updates_pattern == -1:
            ...
            local_checkpoints = checkpoints
            local_checkpoints = [x[0] for x in local_checkpoints if x[1] % cfg.keep_interval_updates_pattern != 0]

        for old_chk in local_checkpoints[cfg.keep_interval_updates:]:
    ...
    if cfg.keep_last_epochs > 0:
        ...
        local_checkpoints = checkpoints
        for old_chk in local_checkpoints[cfg.keep_last_epochs :]:
            if os.path.lexists(old_chk):
                os.remove(old_chk)
            elif PathManager.exists(old_chk):
                PathManager.rm(old_chk)
    ...
    if cfg.keep_best_checkpoints > 0:
        ...
        if not cfg.maximize_best_checkpoint_metric:
            local_checkpoints = local_checkpoints[::-1]

        for old_chk in local_checkpoints[cfg.keep_best_checkpoints:]:
            if os.path.lexists(old_chk):
                os.remove(old_chk)
            elif PathManager.exists(old_chk):
                PathManager.rm(old_chk)
...
def checkpoint_paths(path, pattern=r"checkpoint(\d+)\.pt", keep_match=False):
    ...
    if keep_match:
        local_paths = [(os.path.join(path, x[1]), x[0]) for x in sorted(entries, reverse=True)]
    else:
        local_paths = [os.path.join(path, x[1]) for x in sorted(entries, reverse=True)]
    return local_paths
